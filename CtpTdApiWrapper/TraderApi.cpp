...
TRADEAPI_API void   RegOnFrontConnected(TraderSpi* pSpi, CBOnFrontConnected cb)		//当客户端与交易后台建立起通信连接时（还未登录前），该方法被调用。
{
	pSpi->cbOnFrontConnected = cb;
}

TRADEAPI_API void   RegOnFrontDisconnected(TraderSpi* pSpi, CBOnFrontDisconnected cb)		//当客户端与交易后台通信连接断开时，该方法被调用。当发生这个情况后，API会自动重新连接，客户端可不做处理。
{
	pSpi->cbOnFrontDisconnected = cb;
}

TRADEAPI_API void   RegOnHeartBeatWarning(TraderSpi* pSpi, CBOnHeartBeatWarning cb)		//心跳超时警告。当长时间未收到报文时，该方法被调用。
{
...
TRADEAPI_API void   RegRspUserLogin(TraderSpi* pSpi, CBRspUserLogin cb)	//登录请求响应
{
	pSpi->cbRspUserLogin = cb;
}
TRADEAPI_API void   RegRspUserLogout(TraderSpi* pSpi, CBRspUserLogout cb)	//登出请求响应
{
	pSpi->cbRspUserLogout = cb;
}
TRADEAPI_API void   RegRspUserPasswordUpdate(TraderSpi* pSpi, CBRspUserPasswordUpdate cb)	//用户口令更新请求响应
{
	pSpi->cbRspUserPasswordUpdate = cb;
}
TRADEAPI_API void   RegRspTradingAccountPasswordUpdate(TraderSpi* pSpi, CBRspTradingAccountPasswordUpdate cb)	//资金账户口令更新请求响应
{
	pSpi->cbRspTradingAccountPasswordUpdate = cb;
}
TRADEAPI_API void   RegRspUserAuthMethod(TraderSpi* pSpi, CBRspUserAuthMethod cb)	//查询用户当前支持的认证模式的回复
{
	pSpi->cbRspUserAuthMethod = cb;
}
TRADEAPI_API void   RegRspGenUserCaptcha(TraderSpi* pSpi, CBRspGenUserCaptcha cb)	//获取图形验证码请求的回复
{
	pSpi->cbRspGenUserCaptcha = cb;
}
TRADEAPI_API void   RegRspGenUserText(TraderSpi* pSpi, CBRspGenUserText cb)	//获取短信验证码请求的回复
{
	pSpi->cbRspGenUserText = cb;
}
TRADEAPI_API void   RegRspOrderInsert(TraderSpi* pSpi, CBRspOrderInsert cb)	//报单录入请求响应
{
	pSpi->cbRspOrderInsert = cb;
}
TRADEAPI_API void   RegRspParkedOrderInsert(TraderSpi* pSpi, CBRspParkedOrderInsert cb)	//预埋单录入请求响应
{
	pSpi->cbRspParkedOrderInsert = cb;
}
TRADEAPI_API void   RegRspParkedOrderAction(TraderSpi* pSpi, CBRspParkedOrderAction cb)	//预埋撤单录入请求响应
{
	pSpi->cbRspParkedOrderAction = cb;
}
TRADEAPI_API void   RegRspOrderAction(TraderSpi* pSpi, CBRspOrderAction cb)	//报单操作请求响应
{
	pSpi->cbRspOrderAction = cb;
}
TRADEAPI_API void   RegRspQueryMaxOrderVolume(TraderSpi* pSpi, CBRspQueryMaxOrderVolume cb)	//查询最大报单数量响应
{
	pSpi->cbRspQueryMaxOrderVolume = cb;
}
TRADEAPI_API void   RegRspSettlementInfoConfirm(TraderSpi* pSpi, CBRspSettlementInfoConfirm cb)	//投资者结算结果确认响应
{
	pSpi->cbRspSettlementInfoConfirm = cb;
}
TRADEAPI_API void   RegRspRemoveParkedOrder(TraderSpi* pSpi, CBRspRemoveParkedOrder cb)	//删除预埋单响应
{
	pSpi->cbRspRemoveParkedOrder = cb;
}
TRADEAPI_API void   RegRspRemoveParkedOrderAction(TraderSpi* pSpi, CBRspRemoveParkedOrderAction cb)	//删除预埋撤单响应
{
	pSpi->cbRspRemoveParkedOrderAction = cb;
}
TRADEAPI_API void   RegBatchOrderAction(TraderSpi* pSpi, CBRspBatchOrderAction cb)	//批量报单操作请求响应
{
	pSpi->cbRspBatchOrderAction = cb;
}
TRADEAPI_API void   RegCombActionInsert(TraderSpi* pSpi, CBRspCombActionInsert cb)	//申请组合录入请求响应
{
	pSpi->cbRspCombActionInsert = cb;
}
TRADEAPI_API void   RegRspQryOrder(TraderSpi* pSpi, CBRspQryOrder cb)	//请求查询报单响应
{
	pSpi->cbRspQryOrder = cb;
}
TRADEAPI_API void   RegRspQryTrade(TraderSpi* pSpi, CBRspQryTrade cb)	//请求查询成交响应
{
	pSpi->cbRspQryTrade = cb;
}
TRADEAPI_API void   RegRspQryInvestorPosition(TraderSpi* pSpi, CBRspQryInvestorPosition cb)	//请求查询投资者持仓响应
{
	pSpi->cbRspQryInvestorPosition = cb;
}
TRADEAPI_API void   RegRspQryTradingAccount(TraderSpi* pSpi, CBRspQryTradingAccount cb)	//请求查询资金账户响应
{
	pSpi->cbRspQryTradingAccount = cb;
}
TRADEAPI_API void   RegRspQryInvestor(TraderSpi* pSpi, CBRspQryInvestor cb)	//请求查询投资者响应
{
	pSpi->cbRspQryInvestor = cb;
}
TRADEAPI_API void   RegRspQryTradingCode(TraderSpi* pSpi, CBRspQryTradingCode cb)	//请求查询交易编码响应
{
	pSpi->cbRspQryTradingCode = cb;
}
TRADEAPI_API void   RegRspQryInstrumentMarginRate(TraderSpi* pSpi, CBRspQryInstrumentMarginRate cb)	//请求查询合约保证金率响应
{
	pSpi->cbRspQryInstrumentMarginRate = cb;
}
TRADEAPI_API void   RegRspQryInstrumentCommissionRate(TraderSpi* pSpi, CBRspQryInstrumentCommissionRate cb)	//请求查询合约手续费率响应
{
	pSpi->cbRspQryInstrumentCommissionRate = cb;
}
TRADEAPI_API void   RegRspQryExchange(TraderSpi* pSpi, CBRspQryExchange cb)	//请求查询交易所响应
{
	pSpi->cbRspQryExchange = cb;
}
TRADEAPI_API void   RegRspQryInstrument(TraderSpi* pSpi, CBRspQryInstrument cb)	//请求查询合约响应
{
	pSpi->cbRspQryInstrument = cb;
}
TRADEAPI_API void   RegRspQryDepthMarketData(TraderSpi* pSpi, CBRspQryDepthMarketData cb)	//请求查询行情响应
{
	pSpi->cbRspQryDepthMarketData = cb;
}
TRADEAPI_API void   RegRspQrySettlementInfo(TraderSpi* pSpi, CBRspQrySettlementInfo cb)	//请求查询投资者结算结果响应
{
	pSpi->cbRspQrySettlementInfo = cb;
}
TRADEAPI_API void   RegRspQryTransferBank(TraderSpi* pSpi, CBRspQryTransferBank cb)	//请求查询转帐银行响应
{
	pSpi->cbRspQryTransferBank = cb;
}
TRADEAPI_API void   RegRspQryInvestorPositionDetail(TraderSpi* pSpi, CBRspQryInvestorPositionDetail cb)	//请求查询投资者持仓明细响应
{
	pSpi->cbRspQryInvestorPositionDetail = cb;
}
TRADEAPI_API void   RegRspQryNotice(TraderSpi* pSpi, CBRspQryNotice cb)	//请求查询客户通知响应
{
	pSpi->cbRspQryNotice = cb;
}
TRADEAPI_API void   RegRspQrySettlementInfoConfirm(TraderSpi* pSpi, CBRspQrySettlementInfoConfirm cb)	//请求查询结算信息确认响应
{
	pSpi->cbRspQrySettlementInfoConfirm = cb;
}
TRADEAPI_API void   RegRspQryInvestorPositionCombineDetail(TraderSpi* pSpi, CBRspQryInvestorPositionCombineDetail cb)	//请求查询投资者持仓明细响应
{
	pSpi->cbRspQryInvestorPositionCombineDetail = cb;
}
TRADEAPI_API void   RegRspQryCFMMCTradingAccountKey(TraderSpi* pSpi, CBRspQryCFMMCTradingAccountKey cb)	//查询保证金监管系统经纪公司资金账户密钥响应
{
	pSpi->cbRspQryCFMMCTradingAccountKey = cb;
}
TRADEAPI_API void   RegRspQryEWarrantOffset(TraderSpi* pSpi, CBRspQryEWarrantOffset cb)	//请求查询仓单折抵信息
{
	pSpi->cbRspQryEWarrantOffset = cb;
}
TRADEAPI_API void   RegRspQryInvestorProductGroupMargin(TraderSpi* pSpi, CBRspQryInvestorProductGroupMargin cb)	//请求查询投资者品种/跨品种保证金
{
	pSpi->cbRspQryInvestorProductGroupMargin = cb;
}
TRADEAPI_API void   RegRspQryExchangeMarginRate(TraderSpi* pSpi, CBRspQryExchangeMarginRate cb)	//请求查询交易所保证金率
{
	pSpi->cbRspQryExchangeMarginRate = cb;
}
TRADEAPI_API void   RegRspQryExchangeMarginRateAdjust(TraderSpi* pSpi, CBRspQryExchangeMarginRateAdjust cb)	//请求查询交易所调整保证金率
{
	pSpi->cbRspQryExchangeMarginRateAdjust = cb;
}
TRADEAPI_API void   RegRspQryExchangeRate(TraderSpi* pSpi, CBRspQryExchangeRate cb)	//请求查询汇率响应
{
	pSpi->cbRspQryExchangeRate = cb;
}
TRADEAPI_API void   RegRspQrySecAgentACIDMap(TraderSpi* pSpi, CBRspQrySecAgentACIDMap cb)	//请求查询二级代理操作员银期权限响应
{
	pSpi->cbRspQrySecAgentACIDMap = cb;
}
TRADEAPI_API void   RegRspQryProductExchRate(TraderSpi* pSpi, CBRspQryProductExchRate cb)	//请求查询产品报价汇率
{
	pSpi->cbRspQryProductExchRate = cb;
}
TRADEAPI_API void   RegRspQryProductGroup(TraderSpi* pSpi, CBRspQryProductGroup cb)	//请求查询产品组
{
	pSpi->cbRspQryProductGroup = cb;
}
TRADEAPI_API void   RegRspQryMMInstrumentCommissionRate(TraderSpi* pSpi, CBRspQryMMInstrumentCommissionRate cb)	//请求查询做市商合约手续费率响应
{
	pSpi->cbRspQryMMInstrumentCommissionRate = cb;
}
TRADEAPI_API void   RegRspQryMMOptionInstrCommRate(TraderSpi* pSpi, CBRspQryMMOptionInstrCommRate cb)	//请求查询做市商期权合约手续费响应
{
	pSpi->cbRspQryMMOptionInstrCommRate = cb;
}
TRADEAPI_API void   RegRspQryInstrumentOrderCommRate(TraderSpi* pSpi, CBRspQryInstrumentOrderCommRate cb)	//请求查询报单手续费响应
{
	pSpi->cbRspQryInstrumentOrderCommRate = cb;
}
TRADEAPI_API void   RegRspQrySecAgentTradingAccount(TraderSpi* pSpi, CBRspQrySecAgentTradingAccount cb)	//请求查询资金账户响应
{
	pSpi->cbRspQrySecAgentTradingAccount = cb;
}
TRADEAPI_API void   RegRspQrySecAgentCheckMode(TraderSpi* pSpi, CBRspQrySecAgentCheckMode cb)	//请求查询二级代理商资金校验模式响应
{
	pSpi->cbRspQrySecAgentCheckMode = cb;
}
TRADEAPI_API void   RegRspQrySecAgentTradeInfo(TraderSpi* pSpi, CBRspQrySecAgentTradeInfo cb)	//请求查询二级代理商信息响应
{
	pSpi->cbRspQrySecAgentTradeInfo = cb;
}
TRADEAPI_API void   RegRspQryOptionInstrTradeCost(TraderSpi* pSpi, CBRspQryOptionInstrTradeCost cb)	//请求查询期权交易成本响应
{
	pSpi->cbRspQryOptionInstrTradeCost = cb;
}
TRADEAPI_API void   RegRspQryOptionInstrCommRate(TraderSpi* pSpi, CBRspQryOptionInstrCommRate cb)	//请求查询期权合约手续费响应
{
	pSpi->cbRspQryOptionInstrCommRate = cb;
}
TRADEAPI_API void   RegRspQryExecOrder(TraderSpi* pSpi, CBRspQryExecOrder cb)	//请求查询执行宣告响应
{
	pSpi->cbRspQryExecOrder = cb;
}
TRADEAPI_API void   RegRspQryForQuote(TraderSpi* pSpi, CBRspQryForQuote cb)	//请求查询询价响应
{
	pSpi->cbRspQryForQuote = cb;
}
TRADEAPI_API void   RegRspQryQuote(TraderSpi* pSpi, CBRspQryQuote cb)	//请求查询报价响应
{
	pSpi->cbRspQryQuote = cb;
}
TRADEAPI_API void   RegRspQryOptionSelfClose(TraderSpi* pSpi, CBRspQryOptionSelfClose cb)	//请求查询期权自对冲响应
{
	pSpi->cbRspQryOptionSelfClose = cb;
}
TRADEAPI_API void   RegRspQryInvestUnit(TraderSpi* pSpi, CBRspQryInvestUnit cb)	//请求查询投资单元响应
{
	pSpi->cbRspQryInvestUnit = cb;
}
TRADEAPI_API void   RegRspQryCombInstrumentGuard(TraderSpi* pSpi, CBRspQryCombInstrumentGuard cb)	//请求查询组合合约安全系数响应
{
	pSpi->cbRspQryCombInstrumentGuard = cb;
}
TRADEAPI_API void   RegRspQryCombAction(TraderSpi* pSpi, CBRspQryCombAction cb)	//请求查询申请组合响应
{
	pSpi->cbRspQryCombAction = cb;
}
TRADEAPI_API void   RegRspQryTransferSerial(TraderSpi* pSpi, CBRspQryTransferSerial cb)	//请求查询转帐流水响应
{
	pSpi->cbRspQryTransferSerial = cb;
}
TRADEAPI_API void   RegRspQryAccountregister(TraderSpi* pSpi, CBRspQryAccountregister cb)	//请求查询银期签约关系响应
{
	pSpi->cbRspQryAccountregister = cb;
}
TRADEAPI_API void   RegRspError(TraderSpi* pSpi, CBRspError cb)	//错误应答
{
	pSpi->cbRspError = cb;
}
TRADEAPI_API void   RegRtnOrder(TraderSpi* pSpi, CBRtnOrder cb)	//报单通知
{
	pSpi->cbRtnOrder = cb;
}
TRADEAPI_API void   RegRtnTrade(TraderSpi* pSpi, CBRtnTrade cb)	//成交通知
{
	pSpi->cbRtnTrade = cb;
}
TRADEAPI_API void   RegErrRtnOrderInsert(TraderSpi* pSpi, CBErrRtnOrderInsert cb)	//报单录入错误回报
{
	pSpi->cbErrRtnOrderInsert = cb;
}
TRADEAPI_API void   RegErrRtnOrderAction(TraderSpi* pSpi, CBErrRtnOrderAction cb)	//报单操作错误回报
{
	pSpi->cbErrRtnOrderAction = cb;
}
TRADEAPI_API void   RegRtnInstrumentStatus(TraderSpi* pSpi, CBRtnInstrumentStatus cb)	//合约交易状态通知
{
	pSpi->cbRtnInstrumentStatus = cb;
}
TRADEAPI_API void   RegRtnTradingNotice(TraderSpi* pSpi, CBRtnTradingNotice cb)	//交易通知
{
	pSpi->cbRtnTradingNotice = cb;
}
TRADEAPI_API void   RegRtnErrorConditionalOrder(TraderSpi* pSpi, CBRtnErrorConditionalOrder cb)	//提示条件单校验错误
{
	pSpi->cbRtnErrorConditionalOrder = cb;
}
TRADEAPI_API void   RegRspQryContractBank(TraderSpi* pSpi, CBRspQryContractBank cb)	//请求查询签约银行响应
{
	pSpi->cbRspQryContractBank = cb;
}
TRADEAPI_API void   RegRspQryParkedOrder(TraderSpi* pSpi, CBRspQryParkedOrder cb)	//请求查询预埋单响应
{
	pSpi->cbRspQryParkedOrder = cb;
}
TRADEAPI_API void   RegRspQryParkedOrderAction(TraderSpi* pSpi, CBRspQryParkedOrderAction cb)	//请求查询预埋撤单响应
{
	pSpi->cbRspQryParkedOrderAction = cb;
}
TRADEAPI_API void   RegRspQryTradingNotice(TraderSpi* pSpi, CBRspQryTradingNotice cb)	//请求查询交易通知响应
{
	pSpi->cbRspQryTradingNotice = cb;
}
TRADEAPI_API void   RegRspQryBrokerTradingParams(TraderSpi* pSpi, CBRspQryBrokerTradingParams cb)	//请求查询经纪公司交易参数响应
{
	pSpi->cbRspQryBrokerTradingParams = cb;
}
TRADEAPI_API void   RegRspQryBrokerTradingAlgos(TraderSpi* pSpi, CBRspQryBrokerTradingAlgos cb)	//请求查询经纪公司交易算法响应
{
	pSpi->cbRspQryBrokerTradingAlgos = cb;
}
TRADEAPI_API void   RegRtnFromBankToFutureByBank(TraderSpi* pSpi, CBRtnFromBankToFutureByBank cb)	//银行发起银行资金转期货通知
{
	pSpi->cbRtnFromBankToFutureByBank = cb;
}
TRADEAPI_API void   RegRtnFromFutureToBankByBank(TraderSpi* pSpi, CBRtnFromFutureToBankByBank cb)	//银行发起期货资金转银行通知
{
	pSpi->cbRtnFromFutureToBankByBank = cb;
}
TRADEAPI_API void   RegRtnRepealFromBankToFutureByBank(TraderSpi* pSpi, CBRtnRepealFromBankToFutureByBank cb)	//银行发起冲正银行转期货通知
{
	pSpi->cbRtnRepealFromBankToFutureByBank = cb;
}
TRADEAPI_API void   RegRtnRepealFromFutureToBankByBank(TraderSpi* pSpi, CBRtnRepealFromFutureToBankByBank cb)	//银行发起冲正期货转银行通知
{
	pSpi->cbRtnRepealFromFutureToBankByBank = cb;
}
TRADEAPI_API void   RegRtnFromBankToFutureByFuture(TraderSpi* pSpi, CBRtnFromBankToFutureByFuture cb)	//期货发起银行资金转期货通知
{
	pSpi->cbRtnFromBankToFutureByFuture = cb;
}
TRADEAPI_API void   RegRtnFromFutureToBankByFuture(TraderSpi* pSpi, CBRtnFromFutureToBankByFuture cb)	//期货发起期货资金转银行通知
{
	pSpi->cbRtnFromFutureToBankByFuture = cb;
}
TRADEAPI_API void   RegRtnRepealFromBankToFutureByFutureManual(TraderSpi* pSpi, CBRtnRepealFromBankToFutureByFutureManual cb)	//系统运行时期货端手工发起冲正银行转期货请求，银行处理完毕后报盘发回的通知
{
	pSpi->cbRtnRepealFromBankToFutureByFutureManual = cb;
}
TRADEAPI_API void   RegRtnRepealFromFutureToBankByFutureManual(TraderSpi* pSpi, CBRtnRepealFromFutureToBankByFutureManual cb)	//系统运行时期货端手工发起冲正期货转银行请求，银行处理完毕后报盘发回的通知
{
	pSpi->cbRtnRepealFromFutureToBankByFutureManual = cb;
}
TRADEAPI_API void   RegRtnQueryBankBalanceByFuture(TraderSpi* pSpi, CBRtnQueryBankBalanceByFuture cb)	//期货发起查询银行余额通知
{
	pSpi->cbRtnQueryBankBalanceByFuture = cb;
}
TRADEAPI_API void   RegErrRtnBankToFutureByFuture(TraderSpi* pSpi, CBErrRtnBankToFutureByFuture cb)	//期货发起银行资金转期货错误回报
{
	pSpi->cbErrRtnBankToFutureByFuture = cb;
}
TRADEAPI_API void   RegErrRtnFutureToBankByFuture(TraderSpi* pSpi, CBErrRtnFutureToBankByFuture cb)	//期货发起期货资金转银行错误回报
{
	pSpi->cbErrRtnFutureToBankByFuture = cb;
}
TRADEAPI_API void   RegErrRtnRepealBankToFutureByFutureManual(TraderSpi* pSpi, CBErrRtnRepealBankToFutureByFutureManual cb)	//系统运行时期货端手工发起冲正银行转期货错误回报
{
	pSpi->cbErrRtnRepealBankToFutureByFutureManual = cb;
}
TRADEAPI_API void   RegErrRtnRepealFutureToBankByFutureManual(TraderSpi* pSpi, CBErrRtnRepealFutureToBankByFutureManual cb)	//系统运行时期货端手工发起冲正期货转银行错误回报
{
	pSpi->cbErrRtnRepealFutureToBankByFutureManual = cb;
}
TRADEAPI_API void   RegErrRtnQueryBankBalanceByFuture(TraderSpi* pSpi, CBErrRtnQueryBankBalanceByFuture cb)	//期货发起查询银行余额错误回报
{
	pSpi->cbErrRtnQueryBankBalanceByFuture = cb;
}
TRADEAPI_API void   RegRtnRepealFromBankToFutureByFuture(TraderSpi* pSpi, CBRtnRepealFromBankToFutureByFuture cb)	//期货发起冲正银行转期货请求，银行处理完毕后报盘发回的通知
{
	pSpi->cbRtnRepealFromBankToFutureByFuture = cb;
}
TRADEAPI_API void   RegRtnRepealFromFutureToBankByFuture(TraderSpi* pSpi, CBRtnRepealFromFutureToBankByFuture cb)	//期货发起冲正期货转银行请求，银行处理完毕后报盘发回的通知
{
	pSpi->cbRtnRepealFromFutureToBankByFuture = cb;
}
TRADEAPI_API void   RegRspFromBankToFutureByFuture(TraderSpi* pSpi, CBRspFromBankToFutureByFuture cb)	//期货发起银行资金转期货应答
{
	pSpi->cbRspFromBankToFutureByFuture = cb;
}
TRADEAPI_API void   RegRspFromFutureToBankByFuture(TraderSpi* pSpi, CBRspFromFutureToBankByFuture cb)	//期货发起期货资金转银行应答
{
	pSpi->cbRspFromFutureToBankByFuture = cb;
}
TRADEAPI_API void   RegRspQueryBankAccountMoneyByFuture(TraderSpi* pSpi, CBRspQueryBankAccountMoneyByFuture cb)	//期货发起查询银行余额应答
{
	pSpi->cbRspQueryBankAccountMoneyByFuture = cb;
}
TRADEAPI_API void   RegRspQueryCFMMCTradingAccountToken(TraderSpi* pSpi, CBRspQueryCFMMCTradingAccountToken cb)	//查询监控中心用户令牌应答
{
	pSpi->cbRspQueryCFMMCTradingAccountToken = cb;
}
TRADEAPI_API void   RegRtnCFMMCTradingAccountToken(TraderSpi* pSpi, CBRtnCFMMCTradingAccountToken cb)	//查询监控中心用户令牌通知
{
